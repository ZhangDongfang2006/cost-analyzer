...
名称格式: 辅助材料（出线柜，N路出线）""", language="text")
        ...
        with st.expander("💰 7. 利润计算（阶梯利润率）", expanded=False):
            st.code("""成套费(元) = 各项利润之和
  = 元器件利润 + 铜排利润 + 辅材利润 + 箱体利润

利润率阶梯（按各项元器件金额分别计算）:
  元器件金额(元) < 10,000    → 元器件金额 × 30%
  元器件金额(元) 10,000~50,000  → (0.29 - 0.000003125 × 元器件金额) × 元器件金额
  元器件金额(元) > 50,000    → 元器件金额 × 10%

铜排/辅材/箱体 同理，各自按上述阶梯计算""", language="text")
            st.markdown("- 整体利润率 = 成套费(元) ÷ 总成本(元) × 100%")
            st.markdown("- 成套费包含：组装费 + 企业管理费 + 税收 + 利润")

        # 8. 最终报价
        with st.expander("🏷️ 8. 最终报价", expanded=False):
            st.code("""不含税报价(元) = 总成本(元) + 成套费(元)
含税报价(元) = 不含税报价(元) × 1.13""", language="text")
